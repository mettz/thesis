...
exports.GAS_FORMULAE = {
  ...
  MLOAD: (pointer) => [3, `malloc(${pointer}, 32)`],
  MSTORE: (pointer, variable) => [3, `malloc(${pointer}, sizeof(${variable}))`],
  MSTORE8: (pointer, variable) => [
    3,
    `malloc(${pointer}, sizeof(${variable}))`,
  ],
  SSTORE: (slot, value) => `sstore(${slot}, ${value})`,
  LOG0: (memOffset, memLength) => [
    375,
    `malloc(${memOffset}, ${memLength})`,
  ...
  CREATE: (pointer, length) => [32000, `malloc(${pointer}, ${length})`],
  ...
  CALL: (inOffset, inLength, outOffset, outLength, value, toAddr) => [
    700,
    `malloc(${inOffset}, ${inLength})`,
    `malloc(${outOffset}, ${outLength})`,

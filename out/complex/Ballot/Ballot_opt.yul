...
object "Ballot_314" {
    code {
        ...
        function allocate_memory_1282() -> memPtr
        {
            memPtr := mload(64)
        ...
        function constructor_Ballot(var_proposalNames_mpos)
        {
            ...
            }
            {
                ...
                let expr_mpos := allocate_memory_1282()
                mstore(expr_mpos, returnValue)
                mstore(add(expr_mpos, _3), _1)
                array_push_from_struct_Proposal_to_array_struct_Proposal_storage_dyn_ptr(expr_mpos)
            }
        }
        ...
        code {
            {
                ...
                if iszero(lt(calldatasize(), 4))
                {
                    ...
                    case 0x0121b93f {
                        ...
                        fun_vote(abi_decode_uint256(calldatasize()))
                        return(mload(_1), _2)
                    }
                    case 0x013cf08b {
                        if callvalue() { revert(_2, _2) }
                        let ret, ret_1 := getter_fun_proposals(abi_decode_uint256(calldatasize()))
                        let memPos := mload(_1)
                    ...
                    case 0x5c19a95c {
                        if callvalue() { revert(_2, _2) }
                        fun_delegate(abi_decode_address(calldatasize()))
                        return(mload(_1), _2)
                    }
                    case 0x609ff1bd {
                        if callvalue() { revert(_2, _2) }
                    ...
                    case 0x9e7b8d61 {
                        if callvalue() { revert(_2, _2) }
                        fun_giveRightToVote(abi_decode_address(calldatasize()))
                        return(mload(_1), _2)
                    }
                    case 0xa3ec138d {
                        if callvalue() { revert(_2, _2) }
                        let ret_4, ret_5, ret_6, ret_7 := getter_fun_voters(abi_decode_address(calldatasize()))
            ...
            {
                let _1 := mapping_index_access_mapping_address_struct_Voter_storage_of_address(caller())
                let _2 := 0x01
                let _3 := add(_1, _2)
                ...
                for { } _2 { }
                {
                    if iszero(cleanup_from_storage_address(read_from_storage_split_offset_address(add(mapping_index_access_mapping_address_struct_Voter_storage_of_address(var_to), _2)))) { break }
                    var_to := read_from_storage_split_offset_address(add(mapping_index_access_mapping_address_struct_Voter_storage_of_address(var_to), _2))
                    require_helper_stringliteral_8bd7(iszero(eq(and(var_to, _4), caller())))
                }
                update_storage_value_offsett_bool_to_bool(_3)
                update_storage_value_offsett_address_to_address(_3, var_to)
                let _5 := mapping_index_access_mapping_address_struct_Voter_storage_of_address(var_to)
                switch extract_from_storage_value_offsett_bool(sload(add(_5, _2)))
                case 0 {
                    sstore(_5, checked_add_uint256(sload(_5), sload(_1)))
                }
                default {
                    let _6 := sload(_1)
                    let _7, _8 := storage_array_index_access_struct_Proposal_storage_dyn(sload(add(_5, 0x02)))
                    let _9 := add(_7, _2)
            ...
            {
                if iszero(eq(caller(), and(sload(0x00), sub(shl(160, 1), 1))))
                {
                    let memPtr := mload(64)
                ...
                require_helper_stringliteral_d39b(cleanup_bool(iszero(and(sload(add(mapping_index_access_mapping_address_struct_Voter_storage_of_address(var_voter), 0x01)), 0xff))))
                require_helper(iszero(sload(mapping_index_access_mapping_address_struct_Voter_storage_of_address(var_voter))))
                update_storage_value_offsett_uint256_to_uint256(mapping_index_access_mapping_address_struct_Voter_storage_of_address(var_voter))
            }
            function fun_vote(var_proposal)
            {
                ...
                update_storage_value_offsett_bool_to_bool(_1)
                sstore(add(dataSlot, 2), var_proposal)
                let _2 := sload(dataSlot)
                let _3, _4 := storage_array_index_access_struct_Proposal__dyn(var_proposal)
                let _5 := add(_3, 0x01)
                sstore(_5, checked_add_uint256(sload(_5), _2))
            }
            function fun_winnerName() -> var_winnerName
            {
                let _1, _2 := storage_array_index_access_struct_Proposal_storage_dyn(fun_winningProposal())
            ...
            function fun_winningProposal() -> var_winningProposal
            {
                ...
                {
                    let _2, _3 := storage_array_index_access_struct_Proposal_storage_dyn(var_p)
                    let _4 := 1
                    if gt(sload(add(_2, _4)), var_winningVoteCount)
                    {
                        let _5, _6 := storage_array_index_access_struct_Proposal_storage_dyn(var_p)
                        var_winningVoteCount := sload(add(_5, _4))
            ...
            {
                if iszero(lt(key, sload(2))) { revert(ret, ret) }
                let slot, offset := storage_array_index_access_struct_Proposal__dyn(key)
                ret := sload(slot)
            ...
            function getter_fun_voters(key) -> ret, ret_1, ret_2, ret_3
            {
                ...
                mstore(ret_3, and(key, _1))
                mstore(0x20, 1)
                let dataSlot := keccak256(ret_3, 0x40)
                ret := sload(dataSlot)
            ...
            function mapping_index_access_mapping_address_struct_Voter_storage_of_address(key) -> dataSlot
            {
            ...
            function storage_array_index_access_struct_Proposal_storage_dyn(index) -> slot, offset
            {
                if iszero(lt(index, sload(0x02)))
                {
            ...
            function update_storage_value_offsett_uint256_to_uint256(slot)
            { sstore(slot, 0x01) }
